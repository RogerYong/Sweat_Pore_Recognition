...
def _transform_mini_batch(sample):
  ...
  for image in sample:
    ...
    if(index2 % 3 == 1):
      ...
      transformed.append(h_flip)
      cv2.imwrite(
        "F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/" + str(index) + "hf.jpg",
        h_flip*255)
      index = index + 1
    if (index2 % 3 == 2):
      v_flip = cv2.flip(image, 0)
      transformed.append(v_flip)
      cv2.imwrite(
        "F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/" + str(index) + "vf.jpg",
        v_flip*255)
      index = index + 1
    if (index2 % 3 == 0):
      hv_flip = cv2.flip(image, -1)
      transformed.append(hv_flip)
      cv2.imwrite(
        "F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/" + str(index) + "hvf.jpg",
        hv_flip*255)
      index = index + 1
    # random blur
    ...
    cv2.imwrite(
      "F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/" + str(index) + "b.jpg",
      image*255)
    index = index + 1
    #edge blur
    M1 = image
    ...
    M1[:, 15] = dst[:, 15]
    M1[:, 16] = dst[:, 16]
    transformed.append(M1)
    cv2.imwrite(
      "F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/" + str(index) + "Eb.jpg",
      M1*255)
    index = index + 1
    # random translation
    ...
    #cv2.waitKey()
    image=image*255
    cv2.imwrite("F:/PyCharmProject/fingerprintPoreExtaction/high-res-fingerprint-recognition/imgs/patch/"+str(index)+".jpg", image)
    index = index + 1
    index2 = index2 + 1
    transformed.append(image)

  return np.array(transformed)
#数据增广contrast and brightness variations,translations and rotations
...
def nms(centers, probs, bb_size, thr):
  ...
  #将中心s的每个中心转换为以中心为中心的bb_size x bb_size边界框，并对它们应用非最大值抑制。
  Args:
    centers: centers of detection bounding boxes.检测出的所有bbox
    probs: probabilities for each of the detections.所有bbox的概率
    bb_size: bounding box size.
    thr: NMS discarding intersection threshold.

  Returns:
    dets: np.array of NMS filtered detections.NMS过滤后的点
    det_probs: np.array of corresponding detection probabilities.相应的概率
  '''
...
def pairwise_distances(x1, x2):
  ...
  return D
#计算距离
def pairwise_distancesx(x, y, scope=None):
    with tf.op_scope([x, y], scope, 'pairwise_distancesx'):
        size_x = tf.shape(x)[0]
        size_y = tf.shape(y)[0]
